...
เอกสารนี้รวมคำสั่ง SQL สำหรับสร้างโครงสร้างฐานข้อมูล RoomKub (MySQL) พร้อมคำอธิบายว่าแต่ละส่วนใช้ทำอะไร

หมายเหตุสำคัญ:
- โครงสร้างอ้างอิงจาก `backend/prisma/schema.prisma`
- ไฟล์นี้ใช้สำหรับอ่าน/ทำความเข้าใจและวาด ER Diagram ได้ง่าย
- หากใช้งานจริงในโปรเจกต์ ให้ยึด Prisma Migration เป็นแหล่งจริง (source of truth)

## 1) คำสั่งเตรียมฐานข้อมูล
...
โค้ดนี้คืออะไร:
- สร้างฐานข้อมูลชื่อ `roomkub_prod` หากยังไม่มี
- ตั้ง charset/collation ให้รองรับภาษาไทยและ unicode
- เลือกฐานข้อมูลที่จะสร้างตารางต่อ

---

## 2) คำสั่งสร้างตารางทั้งหมด

```sql
-- 1) role: เก็บบทบาทผู้ใช้ (admin, tenant)
CREATE TABLE role (
  role_id INT AUTO_INCREMENT PRIMARY KEY,
  role_name VARCHAR(191) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- 2) user: เก็บบัญชีผู้ใช้
CREATE TABLE user (
...
-- 4) utility_rate: เรทค่าน้ำ/ค่าไฟ
...
-- 5) billing_cycle: รอบบิลรายเดือนระดับระบบ
...
-- 6) line_group: กลุ่ม LINE สำหรับส่งประกาศ
...
-- 7) knowledge_base: คลังความรู้แบบ Q/A
...
-- 8) knowledge_documents: เมตาดาต้าเอกสารที่อัปโหลดเข้า RAG
...
-- 9) chatbot_conversation_log: ประวัติคำถาม/คำตอบของแชตบอท
CREATE TABLE chatbot_conversation_log (
...
-- 11) audit_log: บันทึกการกระทำผู้ใช้
...
-- 12) user_session: session สำหรับ refresh token
...
-- 13) meter: เก็บมิเตอร์น้ำไฟรายเดือนต่อห้อง
CREATE TABLE meter (
...
-- 16) billing_draft: ร่างบิลก่อนยืนยันสร้างบิลจริง
...
-- 17) notification: แจ้งเตือนรายผู้เช่า
...
-- 18) notification_history: ประวัติการ broadcast ไป LINE group
...
-- 19) knowledge_chunks: chunk ของเอกสารในระบบ RAG
...
โค้ดนี้คืออะไร:
- เป็น DDL (`Data Definition Language`) สำหรับ “สร้างโครงสร้างตาราง” ทั้งหมด
- ครอบคลุม `PRIMARY KEY`, `FOREIGN KEY`, `UNIQUE`, `INDEX`, `FULLTEXT INDEX`
- ตั้งลำดับสร้างตารางจากแม่ -> ลูก เพื่อไม่ชน FK

---

## 3) ความหมายของคีย์และดัชนีแบบสั้น

- `PRIMARY KEY`: รหัสหลักของแต่ละตาราง (ห้ามซ้ำ, ห้าม null)
- `FOREIGN KEY`: ความสัมพันธ์ข้ามตาราง (ใช้ทำ ERD ได้ตรง)
- `UNIQUE`: กันข้อมูลซ้ำ เช่น username, room_number
- `INDEX`: เร่ง query/filter/sort
- `COMPOSITE INDEX`: index หลายคอลัมน์เพื่อรองรับ query เฉพาะทาง
- `FULLTEXT INDEX`: ค้นหาข้อความยาว (ใช้กับ `knowledge_base`)

---

## 4) คำแนะนำใช้งานจริง (Best Practice)

1. ใช้ migration ของ Prisma เป็นตัว deploy schema จริงทุก environment
2. อย่าแก้โครงสร้าง production ด้วย SQL มือเปล่าโดยไม่มี migration ไฟล์
3. สำหรับฟิลด์เงิน (`DECIMAL`) ควรกำหนด precision/scale ให้เหมาะกับธุรกิจ (เช่น `DECIMAL(12,2)`) ใน migration รอบถัดไป
4. ทดสอบ FK/unique/index หลังเปลี่ยน schema เสมอ
5. ถ้าต้องการ ERD อัตโนมัติ ให้ใช้ schema นี้ร่วมกับ dbdiagram/DrawSQL ได้ทันที

---

## 5) คำสั่ง SQL สำหรับ 10 Views (รายงาน)

```sql
-- View 1: สรุปรายได้รายเดือน (ยอดวางบิล + ยอดเก็บจริง)
...
-- View 2: สรุปบิลค้างชำระรายเดือน
...
-- View 4: อัตราความสำเร็จการชำระเงินรายเดือน
...
-- View 6: สรุปอัตราเข้าพักตามประเภทห้อง
...
-- View 7: ปริมาณใช้น้ำรายห้องรายเดือน
...
-- View 8: ปริมาณใช้ไฟรายห้องรายเดือน
...
SELECT
  ...
  CAST(m.electric_meter AS DECIMAL(12, 2))
    - CAST(COALESCE(pm.electric_meter, 0) AS DECIMAL(12, 2)) AS electric_usage_units
FROM meter m
JOIN room r ON r.room_id = m.room_id
LEFT JOIN meter pm ON pm.room_id = m.room_id
  AND pm.month = DATE_FORMAT(
...
-- View 9: แจ้งเตือนการใช้น้ำไฟสูงผิดปกติ
...
-- View 10: สรุปอัตราการอ่านแจ้งเตือน
...
โค้ดนี้คืออะไร:
- เป็น SQL สำหรับสร้าง View รายงาน 10 ตัวที่ฝั่งระบบใช้แสดงข้อมูลสรุป
- `DROP VIEW IF EXISTS` ช่วยให้รันซ้ำได้โดยไม่ชนชื่อเดิม
- View ไม่เก็บข้อมูลจริง แต่เป็น query เสมือนเพื่อให้อ่านง่ายและเรียกซ้ำได้สะดวก
